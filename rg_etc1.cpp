...
#define RG_ETC1_ARRAY_SIZE(X) (sizeof(X) / sizeof(decay_array_to_subtype(X)))
   ...
   public:
      ...
      inline component_t operator[] (unsigned int i) const { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }
      inline component_t& operator[] (unsigned int i) { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }

      inline color_quad_u8& set_component(unsigned int i, parameter_t f)
      {
   ...
   struct vec3F
   {
      ...
      inline float operator[] (unsigned int i) const { RG_ETC1_ASSERT(i < 3); return m_s[i]; }

      inline vec3F& operator += (const vec3F& other) { for (unsigned int i = 0; i < 3; i++) m_s[i] += other.m_s[i]; return *this; }

      inline vec3F& operator *= (float s) { for (unsigned int i = 0; i < 3; i++) m_s[i] *= s; return *this; }
   };
